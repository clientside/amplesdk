...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="members.tabs">
		...
		<xsl:if test="$members/pseudoclass">
			<xsl:call-template name="members.tabs.tab">
				<xsl:with-param name="member_type" select="'pseudoclasses'" />
			</xsl:call-template>
		</xsl:if>
		...
		<tr>
			<td id="eTab_{$member_type}" class="members-tab members-tab-normal" tabIndex="0"
				...
				<xsl:choose>
					...
					<xsl:when test="$member_type = 'pseudoclasses'"><xsl:value-of select="$strings/pseudoclasses" /></xsl:when>
				</xsl:choose>
			</td>
		</tr>
	</xsl:template>
	...
	<xsl:template name="pseudoclasses">
		<xsl:param name="members" />
		<table id="ePane_pseudoclasses" cellPadding="0" cellSpacing="0" border="0" width="100%" class="members-list" style="display:none">
		<thead>
			<tr>
				<td nowrap="yes"><b><xsl:value-of select="$strings/pseudoclass_name" /></b></td>
				<td width="100%"><b><xsl:value-of select="$strings/pseudoclass_descr" /></b></td>
			</tr>
		</thead>
		<tbody>
			<xsl:apply-templates select="$members/pseudoclass">
				<xsl:sort data-type="text" select="@name"/>
			</xsl:apply-templates>
		</tbody>
		</table>
