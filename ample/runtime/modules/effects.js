...
function fAMLQuery_play(oElement, oProperties, vDuration, vType, fHandler, sPseudo)
{
	...
	for (var sKey in oProperties)
		if (oProperties.hasOwnProperty(sKey))
			oEffect._data[sName = fUtilities_toCssPropertyName(sKey)]	= [fAMLQuery_animate_parseValue(fAMLQuery_adjustStyleValue(oElementDOM, sName, fBrowser_getStyle(oElementDOM, sName, oStyle))), fAMLQuery_animate_parseValue(fAMLQuery_adjustStyleValue(oElementDOM, sName, '' + oProperties[sKey]))];
...
//				case "ease":
	...
	for (var sKey in oEffect._data)
		...
		{
			oData	= oEffect._data[sKey];
			aValue	= fAMLQuery_animate_sumValue(oData[0], fAMLQuery_animate_mulValue(fAMLQuery_animate_subValue(oData[1], oData[0]), nRatio));
			// Color value
			if (aValue[1] == '#')
				aValue	= ['#', fUtilities_numberToHex(aValue[0][0] * 255) + fUtilities_numberToHex(aValue[0][1] * 255) + fUtilities_numberToHex(aValue[0][2] * 255)];
			else
...
function fAMLQuery_animate_parseValue(sValue) {
		...
		if (aValue[1] == "rgb") {
			...
			if (aValue = sParameters.split(/\s*,\s*/g)) {
				for (var nIndex = 0, oValue, oValueOut = [[], '', '']; nIndex < aValue.length; nIndex++)
					if (oValue = fAMLQuery_animate_parseValue(aValue[nIndex]))
...
function fAMLQuery_animate_sumValue(oValue1, oValue2) {
...
function fAMLQuery_animate_subValue(oValue1, oValue2) {
	if (oValue1[0] instanceof cArray) {
		for (var nIndex = 0, aValue = []; nIndex < oValue1[0].length; nIndex++)
			aValue.push(oValue1[0][nIndex] - oValue2[0][nIndex]);
		return [aValue, oValue1[1], oValue1[2]];
...
function fAMLQuery_animate_mulValue(oValue, nTimes) {
