...
cXULElement_listbody.prototype.$getTagClose	= function() {
    ...
    if (this.parentNode.firstChild instanceof cXULElement_listhead) {
        ...
        for (var nIndex = 0, aItems = this.parentNode.firstChild.childNodes, oItem; oItem = aItems[nIndex]; nIndex++)
        	aHtml.push('<td' + (oItem.attributes["hidden"] == "true" ? ' style="display:none"' : '') + ' class="xul-listcell" style="padding-top:0px;padding-bottom:0px;"><div style="height:1px;' + (oItem.attributes["width"] ? 'width:' + oItem.attributes["width"] + 'px;' : '') + '"></div><div style="height:1px;' + (oItem.attributes["minwidth"] ? 'width:' + oItem.attributes["minwidth"] + 'px' : '') + '"></div></td>');
        aHtml.push('<td></td>');
    	aHtml.push('</tr>');
    	aHtml.push('</tfoot>');
    }
