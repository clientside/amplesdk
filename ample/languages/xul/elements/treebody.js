...
cXULElement_treebody.prototype.$getTagClose	= function() {
    ...
    if (this.parentNode.head) {
        ...
        for (var nIndex = 0, aItems = this.parentNode.head.items, oItem; oItem = aItems[nIndex]; nIndex++)
        	aHtml.push('<td' + (oItem.attributes["hidden"] == "true" ? ' style="display:none"' : '') + ' class="xul-treecell" style="padding-top:0px;padding-bottom:0px;"><div style="height:1px;' + (oItem.attributes["width"] ? 'width:' + oItem.attributes["width"] + 'px;' : '') + '"></div><div style="height:1px;' + (oItem.attributes["minwidth"] ? 'width:' + oItem.attributes["minwidth"] + 'px' : '') + '"></div></td>');
        aHtml.push('<td></td>');
        aHtml.push('</tr>');
        aHtml.push('</tfoot>');
    }
