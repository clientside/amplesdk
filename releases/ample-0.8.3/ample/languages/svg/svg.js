...
/*
		...
		'DOMNodeInsertedIntoDocument':	function(oEvent) {
			...
			if (sValue != "")
				cSVGElement.transform(this.$getContainer(), sValue);
		}
	};
*/
	// presentation
...
if (!!document.namespaces) {
	...
	cSVGElement_rect.prototype.$getTagOpen	= function() {
		var arcSize	= Math.min(this.getAttribute("rx"), this.getAttribute("ry")) / Math.min(this.getAttribute("width"), this.getAttribute("height"));
...
if (!!document.namespaces) {
	...
	cSVGElement_svg.prototype.$getTagOpen	= function() {
		...
		return '<svg2vml:group class="svg-svg' + (this.hasAttribute("class") ? ' ' + this.getAttribute("class") : '')+ '"\
					style="position:relative;display:inline-block;width:' + sWidth + ';height:' + sHeight + ';"\
