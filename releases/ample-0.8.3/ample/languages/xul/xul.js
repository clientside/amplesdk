...
cXULElement.VIEW_TYPE_REGULAR	= 2;    // Element is rendered as not boxed
cXULElement.VIEW_TYPE_GRID		= 4;    // Element is rendered as not boxed but its children are boxed [only grid]

cXULElement.prototype	= new AMLElement;
cXULElement.prototype.AMLElement	= new AMLElement;
cXULElement.prototype.viewType		= cXULElement.VIEW_TYPE_REGULAR;
...
cXULElement.prototype.refresh   = function()
{
    ...
    if (nLength)
    {
        ...
        for (var nIndex = 0; nIndex < nLength; nIndex++)
        {
            ...
            {
                nElements++;
                if (oElement.hasAttribute("flex") && !window.isNaN(oElement.attributes["flex"]))
                    nFlex  += window.parseInt(oElement.attributes["flex"]);
            }
        }
        ...
        if (nElements)
        {
            var oElementDOM    = window.document.getElementById("box_" + this.uniqueID);
			if (this.viewType == cXULElement.VIEW_TYPE_GRID)
				oElementDOM    = window.document.getElementById("box_" + this.parentNode.parentNode.uniqueID);
            ...
            for (var nIndex = 0; nIndex < nLength; nIndex++)
            {
                ...
                if (oElement.viewType != cXULElement.VIEW_TYPE_VIRTUAL)
                {
                    ...
                    {
                        // set heights
                        if (!window.isNaN(oElement.attributes["flex"]))
                            oElementDOM.tBodies[0].rows[nIndex - nVirtual].cells[0].setAttribute("height",  window.Math.ceil(oElement.attributes["flex"] * 100 / nFlex) + "%");
                        else
                    ...
                    else
                    {
                        // set widths
                        if (!window.isNaN(oElement.attributes["flex"]))
                            oElementDOM.tBodies[0].rows[0].cells[nIndex - nVirtual].setAttribute("width", window.Math.ceil(oElement.attributes["flex"] * 100 / nFlex) + "%");
                        else
...
cXULElement.prototype.$getTag		= function()
{
	...
	if (this.viewType != cXULElement.VIEW_TYPE_VIRTUAL)
		aHtml[aHtml.length]	= this.$getTagOpen();
	...
	for (var nIndex = 0; nIndex < this.childNodes.length; nIndex++)
	{
		...
		if (this.viewType == cXULElement.VIEW_TYPE_BOXED || this.viewType == cXULElement.VIEW_TYPE_GRID)
...
{
    var aHtml   = ['<table cellPadding="0" cellSpacing="0" border="0" id="' + "box_" + oElement.uniqueID + '"'];
...
cXULElement.getBoxOpenChild = function(oElement)
{
    ...
    if (oElement.parentNode.attributes["debug"] == "true")
		aHtml[aHtml.length]	= ' class="xul-box_debug xul-' + (oElement.parentNode.attributes["orient"] != "vertical" ? "h" : "v") + 'box_debug"';
...
cXULPopupElement.prototype.showPopup	= function(oElement, nLeft, nTop, nType, oAnchor, sAlign)
{
    ...
    if ((!window.isNaN(nLeft) && nLeft !=-1) || (!window.isNaN(nTop) && nTop != -1))
    {
    	var oPosition	= this.ownerDocument.$getContainerPosition(window.document.body);
...
cXULSelectElement.getSettingsPopup	= function(oInstance) {
	...
	//
	if (oPopup.parentNode != oInstance.parentNode) {
		oInstance.parentNode.appendChild(oPopup);
...
{
    var sHtml = '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" class="xul-arrowscrollbox">';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
    if (this.attributes["orient"] == "vertical")
    {
...
{
	return sName == "gateway" ? this.$getContainer("box") : window.document.getElementById("box_" + this.uniqueID);
};
...
{
    var sHtml   = '<button id="' + this.uniqueID + '" class="xul-button' +(this.attributes["class"] ? " " + this.attributes["class"] : "") + '"';
...
{
    var sHtml   = '<span id="' + this.uniqueID + '" class="xul-checkbox' + (this.attributes["disabled"] == "true" ? " xul-checkbox_disabled" : "") + '"><input type="checkbox" id="' + this.uniqueID + '_input" name="' + this.attributes["name"] + '"';
    ...
    if (this.attributes["disabled"] == "true")
        sHtml  += ' disabled="true"';
    sHtml  += ' onclick="return ample.$instance(this)._onChange(event);" autocomplete="off" class="xul-checkbox--input" />';
    sHtml  += '<label for="' + this.uniqueID + '_input" class="xul-checkbox--label">' +(this.attributes["label"] ? this.attributes["label"] : '')+ '</label>';
...
cXULElement_colorpicker_pane.prototype.setAttribute	= function(sName, sValue)
{
	if (sName == "value") {
    	...
    	this.$getContainer('colorValue').value    = sValue;
	}
...
cXULElement_colorpicker_pane.prototype._setColor	= function(sColor)
{
    ...
    if (oColor = cXULElement_colorpicker_pane._RGBtoXYB(sColor))
    {
        ...
        this._moveTo('pointerPalette', {'x' : this.x - 9, 'y' : this.y - 9});

        this.$getContainer('colorSample').style.backgroundColor   = sColor;

        this._setColorBrightness(cXULElement_colorpicker_pane._XYBtoRGB({'x': this.x, 'y': this.y, 'b': 0}));
        this._setPaletteBrightness(this.b);

        this._moveTo('pointerBrightness',    {'y' : this.b * 255    - 2});
    }
};

cXULElement_colorpicker_pane.prototype._setColorBrightness	= function(sColor)
{
    this.$getContainer('colorBrightness').style.backgroundColor   = sColor;
    this.$getContainer('shaderBrightness').style.filter   = "progid:DXImageTransform.Microsoft.Gradient(startColorStr='" + sColor + "', endColorStr='#000000', gradientType='0');";
};

cXULElement_colorpicker_pane.prototype._setPaletteBrightness	= function(nBrightness)
{
    // applying styles
    var oElementDOM	= this.$getContainer('shaderPalette');
...
{
    this.$getContainer('colorSample').style.backgroundColor   = sColor;
    this.$getContainer('colorValue').value = sColor;
};
...
{
    var oPosition   = this._getComputedStyleByEvent(oEvent, 'colorBrightness');
    this._moveTo('pointerBrightness', {'y' : oPosition.y - 3});

    this.b  = window.Math.round(100 * oPosition.y / 255) / 100;
    this._setPaletteBrightness(this.b);
...
cXULElement_colorpicker_pane.prototype._onPointersBrightnessMouseDown  = function(oEvent)
{
	...
	window.document.onmousemove  = function(e) {
		return oElement._onPointersBrightnessMouseMove(e || window.event)
	};
...
cXULElement_colorpicker_pane.prototype._onPointerPaletteMouseDown  = function(oEvent)
{
	var oElement	= this;
	window.document.onmousemove  = function(e) {
		return oElement._onPointerPaletteMouseMove(e || window.event)
	};
	window.document.onmouseup    = function() {
		oElement._detachHandlers();
	};
...
{
    this.attributes.value  = this.$getContainer('colorValue').value;
...
// Render
cXULElement_colorpicker_pane.prototype.$getTag	= function()
{
	return '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" class="xul-popup xul-colorpicker-pane">\
				<tbody>\
					<tr>\
						<td id="' + this.uniqueID + '_palette" vAlign="top">\
							<div class="xul-colorpicker-pane-palette-pane xul-colorpicker-pane-border" style="position:relative;" onmousedown="ample.$instance(this)._onPointerPaletteMouseDown(event)">\
								<div id="' + this.uniqueID + '_shaderPalette" class="xul-colorpicker-pane-palette-shader"><br /></div>\
								<div id="' + this.uniqueID + '_pointerPalette" class="xul-colorpicker-pane-palette-pointer" style="position:absolute;"><br /></div>\
							</div>\
						</td>\
						<td align="center" vAlign="top" style="position:relative;display:block;">\
							<div style="width:39px" onmousedown="ample.$instance(this)._onPointersBrightnessMouseDown(event);">\
								<div id="' + this.uniqueID + '_colorBrightness" class="xul-colorpicker-pane-border xul-colorpicker-pane-brightness-pane">\
									<div id="' + this.uniqueID + '_shaderBrightness" class="xul-colorpicker-pane-brightness-shader"><br /></div>\
								</div>\
								<div id="' + this.uniqueID + '_pointerBrightness" class="xul-colorpicker-pane-brightness-pointer" style="position:absolute;left:1px;"><br /></div>\
							</div>\
						</td>\
						<td valign="top">\
							<table cellpadding="0" cellspacing="0" border="0" width="100%" height="100%">\
								<tbody>\
									<tr><td align="right"><div id="' + this.uniqueID + '_colorSample" class="xul-colorpicker-pane-border xul-colorpicker-pane-color-pane"></div></td></tr>\
									<tr><td><br /></td></tr>\
									<tr><td><input id="' + this.uniqueID + '_colorValue" autocomplete="no" type="text" value="#FF0000" maxlength="7" class="xul-colorpicker-pane-border xul-colorpicker-pane-input" onchange="ample.$instance(this)._onInputChange(event, this.value)" onkeyup="ample.$instance(this)._onInputChange(event, this.value)" onselectstart="event.cancelBubble=true;" oncontextmenu="event.cancelBubble=true"/></td></tr>\
									<tr><td><br /></td></tr>\
									<tr><td><button type="button" onclick="ample.$instance(this).acceptDialog();" class="xul-colorpicker-pane-button">OK</button></td></tr>\
									<tr><td height="3"></td></tr>\
									<tr><td><button type="button" onclick="ample.$instance(this).cancelDialog()" class="xul-colorpicker-pane-button">Cancel</button></td></tr>\
								</tbody>\
							</table>\
						</td>\
					</tr>\
...
cXULElement_colorpicker_pane._XYBtoRGB = function(oXYB)
{
    ...
    {
        var nI  = window.Math.floor(nH / 60);
        var nF  = nH / 60 - nI;
        var nP  = nV *(1 - nS);
        var nQ  = nV *(1 - nS * nF);
        var nT  = nV *(1 - nS * (1 - nF));
...
cXULElement_colorpicker_pane._RGBtoXYB = function(sColor)
{
    ...
    var nR  = window.parseInt(sColor.substr(1, 2), 16);
    var nG  = window.parseInt(sColor.substr(3, 2), 16);
    var nB  = window.parseInt(sColor.substr(5, 2), 16);

    var nV  = window.Math.max(nR, nG, nB);
    var nX  = window.Math.min(nR, nG, nB);
    var nS  = (nV-nX) / nV;
...
cXULElement_colorpicker_pane._toHex    = function(nValue)
{
    ...
    if (nValue > 255)
        ...
        return sHexCharacters.charAt(window.Math.floor(nValue / 16)) + sHexCharacters.charAt(nValue % 16);
};
...
{
    var sHtml   = '<table id="' + this.uniqueID + '" class="xul-colorpicker' + (this.attributes["disabled"] == "true" ? " xul-colorpicker_disabled" : "") + '" cellPadding="0" cellSpacing="0" border="0">';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
    sHtml  += '<td width="100%"><div style="height:17px"><input class="xul-colorpicker--input" type="text" autocomplete="off" value="' + this.attributes["value"] + '"' + (this.attributes["disabled"] == "true" ? ' disabled="true"' : '') +' maxlength="7" onchange="ample.$instance(this)._onChange(event)" style="border:0px solid white;width:100%;" onselectstart="event.cancelBubble=true;" /></div></td>';
    sHtml  += '<td valign="top"><div class="xul-colorpicker--button"/></td>';
    sHtml  += '</tr>';
    sHtml  += '</tbody>';
    sHtml  += '</table>';
...
{
    return '<th id="' + this.uniqueID + '" class="xul-column">' +(this.attributes["label"] ? this.attributes["label"] : '');
};
...
{
    return '<thead id="' + this.uniqueID + '" class="xul-columns">';
};
...
cXULElement_datepicker_pane.$getTagDays	= function(oInstance, oDate) {
	...
	for (var nIndex = 1, oDateCurrent; nIndex <= nDays; nIndex++)
	{
...
cXULElement_datepicker_pane.prototype.$getTagOpen	= function() {
	var sHtml = '<table id="' + this.uniqueID + '" class="xul-popup xul-datepicker-pane' + (this.hasAttribute("class") ? ' ' + this.getAttribute("class") : '') + '" cellpadding="0" cellspacing="0" border="0">\
				<thead>\
					<tr>\
...
cXULElement_datepicker.prototype.$getTagOpen	= function() {
	return '<div id="' + this.uniqueID + '" class="xul-datepicker' + (this.hasAttribute("class") ? ' ' + this.getAttribute("class") : '') + (this.getAttribute('disabled') == "true" ? " xul-datepicker_disabled" : "") + '"' + (this.hasAttribute("style") ? ' style="' + this.getAttribute("style") + '"' : '')+ '>\
...
cXULElement_datepicker.prototype.$getTagClose	= function() {
	return '</div>';
};
...
cXULElement_deck.prototype.setAttribute  = function(sName, sValue)
{
    if (sName == "selectedIndex")
    {
        ...
        {
            if (window.isNaN(sValue) || this.childNodes.length < sValue * 1)
                sValue  = "0";
...
cXULElement_deck.handlers	= {
	"DOMNodeInsertedIntoDocument":	function(oEvent) {
		if (!window.isNaN(this.attributes["selectedIndex"]))
...
{
    return '<div id="' + this.uniqueID + '" class="xul-deck">';
};
...
{
    return '<div id="' + this.uniqueID + '" class="xul-description' +(this.attributes["class"] ? " " + this.attributes["class"] : '')+ '" style="width:100%;height:100%;">' + (this.attributes["value"] || "");
};
...
cXULElement_dialog.prototype.centerWindowOnScreen    = function()
{
	...
	oElementDOM.style.left	=(window.document.body.clientWidth - oPosition.width) / 2;
	oElementDOM.style.top	=(window.document.body.clientHeight - oPosition.height) / 2;
};
...
{
	return '<table id="' + this.uniqueID + '" class="xul-dialog '+(this.attributes["class"] ? " " + this.attributes["class"] : "") + '" cellPadding="0" cellSpacing="0" border="0"' +
					...
					<tr>\
						<th class="xul-dialog--head" height="1">\
							<table cellPadding="0" cellSpacing="0" border="0" width="100%">\
								<tbody>\
									<tr><td class="xul-dialog--title">' +(this.attributes["title"] ? this.attributes["title"] : "&nbsp")+ '</td>\
								</tbody>\
							</table>\
						</th>\
					</tr>\
...
{
	return '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" width="100%" class="xul-dialogheader">\
				<tbody>\
					<tr><td vAlign="top"><div class="xul-dialogheader--title">' +(this.attributes["title"] || "&nbsp;")+ '</div><div class="xul-dialogheader--description">' +(this.attributes["description"] || "&nbsp;")+ '</div></td></tr>\
				</tbody>\
			</table>';
};
...
{
    var sHtml   = '<div id="'+this.uniqueID+'" class="xul-editor">';
...
{
	return cXULElement.getBoxOpen(this);
};

// Element Render: close
cXULElement_grid.prototype.$getTagClose	= function()
{
	return cXULElement.getBoxClose(this);
};
...
{
    var sHtml   = '<table id="' + this.uniqueID + '" cellpadding="0" cellspacing="0" border="0"' +(this.attributes["width"] ? ' width="' + this.attributes["width"] + '"' : '')+ '' +(this.attributes["height"] ? ' height="' + this.attributes["height"] + '"' : '')+ ' class="xul-groupbox' +(this.attributes["class"] ? " " + this.attributes["class"] : "") + '">';
    sHtml  += '<thead>';
    sHtml  += '<tr>';
    sHtml  += '<td class="xul-groupbox-head-left"></td>';
    sHtml  += '<td class="xul-groupbox-head"><span id="' + this.uniqueID + '_caption" class="xul-caption" style="display:none;"></span></td>';
...
{
    return '<iframe id="' + this.uniqueID + '" name="' + this.uniqueID + '" class="xul-iframe" height="' +(this.attributes["height"] || '100%')+ '" width="' +(this.attributes["width"] || '100%')+ '" src="' +(this.attributes["src"] || 'about:blank') + '" frameborder="0" border="0" scrolling="no" onload="ample.$instance(this)._onLoad(event)" onunload="ample.$instance(this)._onUnLoad(event)">';
};
...
{
    return '<img id="' + this.uniqueID + '" class="xul-image' +(this.attributes["class"] ? " " + this.attributes["class"] : "") + '"' +(this.attributes["width"] ? ' width="' + this.attributes["width"] + '"' : '')+(this.attributes["height"] ? ' height="' + this.attributes["height"] + '"' : '')+(this.attributes["src"] ? ' src="' + this.attributes["src"] + '"' :'')+ ' onload="ample.$instance(this)._onLoad(event)"/>';
};
...
//        this.$getContainer().setAttribute("for", sValue);
    ...
    if (sName == "class")
    {
        this.$getContainer().className    = "xul-label" +(sValue ? " " + sValue : "");
    }
    else
    {
        this._setAttribute(sName, sValue);
...
//' +(this.attributes["control"] ? ' for="' + this.attributes["control"] + '"': '')+ '
    return '<label id="' + this.uniqueID + '" class="xul-label' +(this.attributes["class"] ? " " + this.attributes["class"] : "")+ '">' +(this.attributes["value"] ? this.attributes["value"] : '');
};
...
{
	return '<tr id="' + this.uniqueID + '"' +(this.attributes["hidden"] == "true" ? ' style="display:hidden;"' : '')+ '>\
				<td style="height:100%">\
					<div id="' + this.uniqueID + '_area" style="height:100%;overflow:scroll;position:relative;" onscroll="return ample.$instance(this)._onScroll(event)">\
...
{
    return '<table id="' + this.uniqueID + '" class="xul-listbox' + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '" cellPadding="0" cellSpacing="0" border="0" height="' +(this.attributes["height"] ? this.attributes["height"] : '100%')+ '" width="' +(this.attributes["width"] ? this.attributes["width"] : '100%')+ '">\
    			<tbody>';
};
...
{
    var sHtml   = '<td id="' + this.uniqueID + '" class="xul-listcell"><div class="xul-listcell--gateway">';
    if (this.attributes["image"])
        sHtml  += '<img src="' + this.attributes["image"] + '" align="absmiddle">&nbsp;';
    if (this.attributes["label"])
        sHtml  += this.attributes["label"];
...
{
    return '<td id="' + this.uniqueID + '" class="xul-listcol"><img height="1"/></td>';
};
...
{
    return '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" width="100%" class="xul-listcols">\
				<tbody>\
					<tr>';
};
...
{
    return '<tr id="' + this.uniqueID + '"' + (this.attributes["hidden"] == "true" ? ' style="display:none"' : '') + '>\
				<td>\
					<div id="' + this.uniqueID + '_area" style="height:18px;overflow:hidden;position:relative;">\
...
/*
	"mousemove":	function(oEvent) {
		var oElementDOM	= this.$getContainer();
		var oPosition	= this.ownerDocument.$getContainerPosition(oElementDOM);
		if (window.Math.abs(oPosition.left - oEvent.clientX) < 10 || window.Math.abs(oPosition.left + oPosition.width - oEvent.clientX) < 10)
...
{
	return '<th id="' + this.uniqueID + '" class="xul-listheader' +(this.attributes["class"] ? " " + this.attributes["class"] : "")+ '"' +(this.attributes["width"] ? ' width="' + this.attributes["width"] + '"' : "")+ ' align="left">\
...
cXULElement_listitem.prototype.$getTagOpen	= function()
{
	...
	return '<tr id="' + this.uniqueID + '" class="xul-listitem' + (this.attributes["class"] ? " xul-listitem_" + this.attributes["class"] : "") + '">' +
...
cXULElement_menu.prototype.setAttribute	= function(sName, sValue)
{
    ...
    if (sName == "label")
    {
        if (this.parentNode instanceof cXULElement_menupopup)
            this.$getContainer().cells[1].innerHTML   = sValue;
        else
            this.$getContainer().innerHTML    = sValue;
	}
	else
...
{
    if (this.parentNode instanceof cXULElement_menupopup)
        return '<tr id="' + this.uniqueID + '" class="xul-menu' + (this.attributes["disabled"] == "true" ? " xul-menu_disabled" : "") + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '">\
        			<td width="18"><div id="' + this.uniqueID + '_control" class="xul-menu-control"' +(this.attributes["image"] ? ' style="background-image:url('+ this.attributes["image"] + ')"' : '')+ '></div></td>\
        			<td noWrap="nowrap">' + (this.attributes["label"] || '&nbsp;')+ '</td>\
        			<td vAlign="top" class="xul-menupopup--gateway">';
    else
        return '	<td vAlign="bottom" class="xul-menu--gateway">';
};
...
cXULElement_menu.prototype.$getTagClose	= function()
{
    if (this.parentNode instanceof cXULElement_menupopup)
        ...
        return '	</td>\
        			<td id="' + this.uniqueID + '" noWrap="nowrap" vAlign="center" class="xul-menu' + (this.attributes["disabled"] == "true" ? " xul-menu_disabled" : "") + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '">' + (this.attributes["label"] || '&nbsp;') + '</td>';
};
...
{
	return '<div id="' + this.uniqueID + '" class="xul-menubar">\
    			<table cellPadding="0" cellSpacing="0" border="0" width="100%">\
    				<tbody>\
...
{
	return '<tr id="' + this.uniqueID + '" class="xul-menuitem' + (this.attributes["disabled"] == "true" ? " xul-menuitem_disabled" : "") + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '">\
...
cXULElement_menulist.prototype.$getTagOpen		= function() {
	return	'<table id="' + this.uniqueID + '" cellpadding="0" cellspacing="0" border="0" class="xul-menulist' +(this.attributes["disabled"] ? " xul-menulist_disabled" : '')+ '">\
				<tbody>\
					<tr>\
...
cXULElement_menupopup.prototype.selectItem	= function(oItem)
{
	...
	if (this.selectedItem && this.selectedItem != oItem) {
		...
		if (this.selectedItem.menupopup) {
			...
			this._timeOutHide	= window.setTimeout(function() {
	...
	if (this._timeOutShow)
		this._timeOutShow	= window.clearTimeout(this._timeOutShow);

	if (oItem) {
	    // Show new element
		if (oItem.menupopup && oItem.$isAccessible()) {
			var oMenuPopupNew	= oItem.menupopup;
			this._timeOutShow	= window.setTimeout(function() {
...
{
	return '<div id="' + this.uniqueID + '" style="position:absolute;' + (this.attributes["hidden"] == "true" ? 'display:none;' : '') + '" class="xul-menupopup">\
...
{
    return '<div id="' + this.uniqueID + '" class="xul-page' + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '" style="width:100%;height:100%;overflow:hidden;">';
};
...
cXULElement_popup.prototype.setAttribute = function(sName, sValue)
{
    if (sName == "top")
    {
        if (!window.isNaN(sValue) && !window.isNaN(this.attributes["left"]))
            this.moveTo(this.attributes["left"] * 1, sValue * 1);
    }
    else
    if (sName == "left")
    {
        if (!window.isNaN(sValue) && !window.isNaN(this.attributes["top"]))
...
{
    return '<div id="' + this.uniqueID + '" style="display:none;position:absolute;width:' + this.attributes["width"] + 'px;" class="xul-popup" onmousedown="event.cancelBubble=true;" oncontextmenu="return false">';
};
...
{
    var sHtml   = '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" width="' + (this.attributes["width"] || "100%") + '" class="xul-progressmeter"' +(this.attributes["hidden"] == "true" ? ' style="display:none;"' : "")+ '>';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
...
{
    var sHtml   = '<span id="' + this.uniqueID + '" class="xul-radio' + ((this.group && this.group.attributes["disabled"] == "true") || this.attributes["disabled"] == "true" ? " xul-radio_disabled" : "")+ '">';
    sHtml	+= '<input type="radio" id="' + this.uniqueID + '_input" autocomplete="off" name="' +(this.group ? this.group.uniqueID : '')+ '_radio"';
    if (this.attributes["value"])
        sHtml  += ' value="' + this.attributes["value"] + '"';
    if (this.group)
    {
        if (this.attributes["value"] && this.attributes["value"] == this.group.attributes["value"])
            this.attributes["selected"]	= "true";

        if (this.attributes["selected"] == "true")
        {
            sHtml  += ' checked="true"';
            this.group.selectedIndex    = this.group.items.length - 1;
            this.group.selectedItem     = this;
            this.group.attributes["value"] = this.attributes["value"];
        }
        if (this.group.attributes["disabled"] == "true" || this.attributes["disabled"] == "true")
            sHtml  += ' disabled="true"';
    }
    sHtml  += ' onclick="ample.$instance(this)._onClick(event)" class="xul-radio--input" />';
    sHtml  += '<label for="' + this.uniqueID + '_input" class="xul-radio--label">' +(this.attributes["label"] ? this.attributes["label"] : '')+ '</label>';
...
{
    return '<div id="' + this.uniqueID + '" class="xul-radiogroup' + (this.attributes["disabled"] == "true" ? " xul-radiogroup_disabled" : "") + '">';
};
...
cXULElement_row.prototype.viewType	= cXULElement.VIEW_TYPE_GRID;
...
{
    return '<tr id="' + this.uniqueID + '" class="xul-row"' +(this.attributes["height"] ? ' height="' + this.attributes["height"] + '"' : '')+(this.attributes["hidden"] == "true" ? ' style="display:none"' : '')+'>';
};
...
{
    if (sValue * 1.0 < this.attributes["min"] * 1.0 || window.isNaN(sValue))
    ...
    this.$getContainer("button").style.left   = window.Math.round((oPosition.width - 17) * (sValue - this.attributes["min"] * 1.0) / (this.attributes["max"] * 1.0 - this.attributes["min"] * 1.0)) + "px";
...
cXULElement_scale.prototype._onButtonMouseDown  = function(oEvent)
{
    ...
    this._buttonX   = window.parseInt(this.$getContainer("button").style.left);
    this._buttonY   = window.parseInt(this.$getContainer("button").style.top);
...
cXULElement_scale._onDocumentMouseUp    = function(oEvent)
{
	...
	var sValue	= oElement.attributes["min"] * 1.0 + window.Math.round((oElement._buttonX  / (oPosition.width - 17)) * (oElement.attributes["max"] * 1.0 - oElement.attributes["min"] * 1.0) / oElement.attributes["step"]) * oElement.attributes["step"];
...
{
    var sHtml   = '<table id="' + this.uniqueID + '" tabindex="0"' +(this.attributes["disabled"] ? ' disabled="disabled"' : '') + ' cellPadding="0" cellSpacing="0" border="0" class="xul-scale' + (this.attributes["disabled"] ? " xul-scale_disabled" : '') + '" ' + (this.attributes["style"] ? ' style="' + this.attributes["style"] + '"': '') + ' onkeydown="return ample.$instance(this)._onButtonKeyDown(event);">';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
    sHtml  += '<td><div style="width:3px" /></td>';
    sHtml  += '<td width="100%" class="xul-scale-bar-horizontal">';
...
cXULElement_script.handlers	= {
	"DOMNodeInsertedIntoDocument":	function(oEvent) {
		...
		if (this.firstChild) {
			var oElement	= window.document.body.appendChild(window.document.createElement("script"));
...
{
    return '<script id="' + this.uniqueID + '" type="text/javascript">';
};
...
{
    return '<div id="' + this.uniqueID + '" class="xul-scrollbox" style="overflow:hidden">';
};
...
{
	return '<div id="'+this.uniqueID+'" class="xul-separator" style="height:1.5em;width:1.5em;"><img height="1" width="1" /></div>';
};
...
{
    var sHtml   = '<div id="'+this.uniqueID+'" class="xul-spacer" style="';
...
{
    var sHtml   = '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" class="xul-spinbuttons' + (this.attributes["disabled"] ? ' xul-spinbuttons_disabled' : '')+ '">';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
...
//	this.ownerDocument.$getContainer("popup").style.display	= "";
	...
	// Save cursor type
	this.cursor	= window.document.body.style.cursor;
	...
	window.document.body.style.cursor	= this.parentNode.attributes["orient"] == "vertical" ? "n-resize" : "e-resize";
};
...
cXULElement_splitter._onDocumentMouseUp   = function(oEvent)
{
	...
	oElement.previousSibling.setBoxObjectParam(sAttribute, (window.Math.round((1 + nOffset1) * window.RegExp.$1) || 1) + window.RegExp.$2);
	oElement.nextSibling.getBoxObjectParam(sAttribute).match(/([0-9]+)(.*)/);
	oElement.nextSibling.setBoxObjectParam(sAttribute, (window.Math.round((1 - nOffset2) * window.RegExp.$1) || 1) + window.RegExp.$2);
	...
	// Restore cursor type
	window.document.body.style.cursor	= oElement.cursor;
...
{
    return '<div id="' + this.uniqueID + '" class="xul-splitter xul-splitter-' +(this.parentNode.attributes["orient"] == "vertical" ? "vertical" : "horizontal")+ '"><img onmousedown="ample.$instance(this)._capture(event); return false;">';
};
...
{
    return '<div id="' + this.uniqueID + '" class="xul-stack" style="position:relative">';
};
...
{
    return '<div id="' + this.uniqueID + '" class="xul-statusbar">';
};
...
{
    var sHtml   = '<div id="' + this.uniqueID + '" class="xul-statusbarpanel">';
    if (this.attributes["image"])
        sHtml  += '<img src="' + this.attributes["image"] + '" align="absmiddle"/>';
    else
    if (this.attributes["label"])
...
{
    var sHtml   = '<td id="' + this.uniqueID + '" class="xul-tab' + (this.attributes["disabled"] ? " xul-tab_disabled" : "") +(this.attributes["class"] ? " " + this.attributes["class"] : "") + '">';
    if (this.attributes["image"])
...
{
    return '<div id="' + this.uniqueID + '" class="xul-tabbox' + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '">';
};
	...
	"DOMNodeInsertedIntoDocument":	function(oEvent) {
		if (this.parentNode instanceof cXULElement_tabpanels) {
			...
			if (oTabBox instanceof cXULElement_tabbox) {
				if (!window.isNaN(oTabBox.attributes["selectedIndex"]) && oTabBox.tabs.items.length > oTabBox.attributes["selectedIndex"] && this.parentNode.items.$indexOf(this) == oTabBox.attributes["selectedIndex"] * 1)
...
{
    return '<div id="'+this.uniqueID+'" class="xul-tabpanel"' +(this.attributes["hidden"] != "false" ? ' style="display:none"' : '')+ '>';
};
...
{
    var sHtml	= '<div id="' + this.uniqueID + '" class="xul-tabpanels">';
    sHtml  += '<table id="' + this.uniqueID + '_table" cellPadding="0" cellSpacing="0" border="0" width="100%" height="100%">';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
    sHtml  += '<td>';
...
{
    var sHtml   = '<div id="' + this.uniqueID + '" class="xul-tabs">';
    sHtml  += '<table id="' + this.uniqueID + '_table" cellPadding="0" cellSpacing="0" border="0">';
    sHtml  += '<tbody>';
    sHtml  += '<tr class="xul-tabs--gateway">';
    sHtml  += '<td class="xul-tab-separator"><img width="1" height="1" /></td>';
...
{
    var sHtml   = '<td class="xul-tab-remainder"><img width="1" height="1" /></td>';
    sHtml  += '</tr>';
    sHtml  += '</tbody>';
    sHtml  += '</table>';
    sHtml  += '</div>';
...
cXULElement_textbox.prototype.$getTagOpen	= function(oElement)
{
    ...
    if (this.attributes["readonly"] == "true")
        sHtml  += ' readonly="true"';
    sHtml  += ' id="' + this.uniqueID + '" name="' + this.attributes["name"] + '" autocomplete="off" class="xul-textbox' +(this.attributes["disabled"] == "true" ? " xul-textbox_disabled" : '')+ '"';
...
cXULElement_timepicker.prototype.$getTagOpen		= function()
{
    ...
    var sHtml   = '<table id="' + this.uniqueID + '" cellpadding="0" cellspacing="0" border="0" class="xul-timepicker' + (this.attributes["disabled"] ? " xul-timepicker_disabled" : '') + '">';
    sHtml  += '<tbody>';
    sHtml  += '<tr>';
...
{
    return '<div id="' + this.uniqueID + '" class="xul-toolbar">';
};
...
cXULElement_toolbarbutton.prototype.$getTagOpen	= function()
{
    ...
    return '<table id="' + this.uniqueID + '" cellPadding="0" cellSpacing="0" border="0" class="xul-toolbarbutton' + (this.attributes["disabled"] == "true" ? " xul-toolbarbutton_disabled" : "") + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '">\
				<tbody>\
					<tr height="3">\
...
{
    return (this.parentNode instanceof cXULElement_menubar ? "<td>" : "") + '<div id="' + this.uniqueID + '" class="xul-toolbargrippy"><br /></div>';
};
...
{
    return '<div id="' + this.uniqueID + '" class="xul-toolbarseparator"><br /></div>';
};
...
{
    return '<div id="' + this.uniqueID + '" class="xul-toolbox">';
};
...
// Render
cXULElement_tooltip_pane.prototype.$getTag	= function()
{
    return '<span id="' + this.uniqueID + '" class="xul-tooltip-pane" style="position:absolute;display:none;"></span>';
};
...
{
    return '<div id="' + this.uniqueID + '" style="display:none;position:absolute;" class="xul-tooltip">';
};
...
{
    return '<table id="' + this.uniqueID + '" class="xul-tree' + (this.attributes["class"] ? " " + this.attributes["class"] : "") + '" cellPadding="0" cellSpacing="0" border="0" height="' +(this.attributes["height"] ? this.attributes["height"] : '100%')+ '" width="' +(this.attributes["width"] ? this.attributes["width"] : '100%')+ '">\
    			<tbody>';
};
...
cXULElement_treebody.prototype.$getTagOpen	= function()
{
    return '<tr id="' + this.uniqueID + '"' +(this.attributes["hidden"] == "true" ? ' style="display:hidden;"' : '')+ '>\
				<td style="height:100%">\
					<div id="' + this.uniqueID + '_area" style="height:100%;width:100%;overflow:scroll;position:relative;" onscroll="return ample.$instance(this)._onScroll(event)">\
...
{
	this.parentNode.parentNode._onNodeClick(oEvent);
};

// Element Render: open
cXULElement_treecell.prototype.$getTagOpen	= function()
{
    var sHtml   = '<td id="' + this.uniqueID + '" class="xul-treecell">';
...
{
	return '<th id="' + this.uniqueID + '" class="xul-treecol ' +(this.attributes["class"] ? " " + this.attributes["class"] : "")+ '"' +(this.attributes["width"] ? ' width="' + this.attributes["width"] + '"' : "")+(this.attributes["hideheader"] == "true" ? ' style="display:none"' : "")+ ' align="left">\
...
cXULElement_treeitem.handlers	= {
	...
	"mousedown":	function(oEvent) {
		...
		if (oEvent.target != this && oEvent.target.parentNode != this && oEvent.target.parentNode.parentNode != this)
			return;
...
// Event Handlers
cXULElement_treeitem.prototype._onNodeClick      = function(oEvent)
{
    this.setAttribute("open", this.getAttribute("open") == "true" ? "false" : "true");
};

cXULElement_treeitem.prototype._onCommandClick   = function(oEvent)
{
    if (this.$getContainer("command").checked)
    {
        if (this.parentNode.tree.attributes["type"] == "radio")
            this.parentNode.tree.selectItem(this);
        else
        if (this.parentNode.tree.attributes["type"] == "checkbox")
            this.parentNode.tree.addItemToSelection(this);
    }
    else
    {
        this.parentNode.tree.removeItemFromSelection(this);
    }
};

// Element Render: open
cXULElement_treeitem.prototype.$getTagOpen	= function()
{
	var oTree	= this.parentNode.tree;
    if (this.parentNode.parentNode.attributes["open"] == "false")
        this.parentNode.attributes["hidden"] = "true";

	return '<tr id="' + this.uniqueID + '" class="xul-treeitem' + (this.attributes["class"] ? " xul-listitem_" + this.attributes["class"] : '') + '"' + (this.parentNode.parentNode.attributes["open"] == "false" ? ' style="display:none"' : '')+ '>' +
	    	(oTree && (oTree.attributes["type"] == "checkbox" || oTree.attributes["type"] == "radio")
			? ('<td style="padding:0" align="center" onmousedown="event.cancelBubble=true">' +
				(oTree.attributes["type"] == "checkbox"
				? '<input type="checkbox" name="' + oTree.uniqueID + '_cmd" class="xul-treeitem--command" onclick="return ample.$instance(this)._onCommandClick(event);" autocomplete="off"/>'
				: (oTree.attributes["type"] == "radio"
					? '<input type="radio" name="' + oTree.uniqueID + '_cmd" class="xul-treeitem--command" onclick="return ample.$instance(this)._onCommandClick(event);"/>'
	        		: '&nbsp;')) +
	        '</td>')
	        : '');
};
...
// Element Render: close
cXULElement_treeitem.prototype.$getTagClose	= function()
{
    return '</tr>';
};
...
cXULElement_treerow.handlers	= {
	"mouseenter":	function(oEvent) {
		this.parentNode.$setPseudoClass("hover", true);
	},
	"mouseleave":	function(oEvent) {
		this.parentNode.$setPseudoClass("hover", false);
	},
	"DOMNodeInserted":	function(oEvent) {
		if (oEvent.target.parentNode == this)
...
//cXULElement_window.handlers.resizestart		= cXULElement_window.handlers.dragstart;
//cXULElement_window.handlers.resizedragend	= cXULElement_window.handlers.dragend;

// Element Renders
cXULElement_window.prototype.$getTagOpen	= function()
{
	return '<table id="' + this.uniqueID + '" class="xul-window '+(this.attributes["class"] ? " " + this.attributes["class"] : "") + '" cellPadding="0" cellSpacing="0" border="0"' +
				(this.attributes["width"] ? ' width="' + this.attributes["width"] + '"' : '') +
				(this.attributes["height"] ? ' height="' + this.attributes["height"] + '"' : '') +
				(this.attributes["hidden"] == "true" ? ' style="display:none;"' : '') + '">\
				<thead ' +(this.attributes["hidechrome"] == "true" ? ' style="display:none"': '')+ '>\
					<tr>\
						<th class="xul-window--head" height="1">\
							<table cellPadding="0" cellSpacing="0" border="0" width="100%">\
								<tbody>\
									<tr>\
										<td class="xul-window--title">' +(this.attributes["title"] ? this.attributes["title"] : "&nbsp")+ '</td>\
...
{
	return '<table id="' + this.uniqueID + '" class="xul-wizard '+(this.attributes["class"] ? " " + this.attributes["class"] : "") + '" cellPadding="0" cellSpacing="0" border="0"' +
				(this.attributes["width"] ? ' width="' + this.attributes["width"] + '"' : '') +
				(this.attributes["height"] ? ' height="' + this.attributes["height"] + '"' : '') +
				(this.attributes["hidden"] == "true" ? ' style="display:none;"' : '') + '">\
				<thead ' +(this.attributes["hidechrome"] == "true" ? ' style="display:none"': '')+ '>\
					<tr>\
						<th class="xul-wizard--head" height="1">\
							<table cellPadding="0" cellSpacing="0" border="0" width="100%">\
								<tbody>\
									<tr><td class="xul-wizard--title">' +(this.attributes["title"] ? this.attributes["title"] : "&nbsp")+ '</td>\
								</tbody>\
							</table>\
						</th>\
					</tr>\
...
{
    return '<div id="' + this.uniqueID + '" class="xul-wizardpage"' +(this.attributes["hidden"] == "true" ? ' style="display:none"' : '')+ '>';
};
