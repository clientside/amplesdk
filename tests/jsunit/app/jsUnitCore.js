...
JsUnitTestSuite.prototype.addTestPage = function (page) {
    this._testPages[this._testPages.length] = JsUnit.Uri.resolveUri(page, document.location.href);
}
...
// added by ilinsky
JsUnit.Uri	= {};
JsUnit.Uri.getUriComponents	= function(sUri) {
	var aResult	= sUri.match(/^(([^:\/?#]+):)?(\/\/([^\/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?/);
	return [aResult[2], aResult[4], aResult[5], aResult[7], aResult[9]];
};

JsUnit.Uri.resolveUri	= function(sUri, sBaseUri) {
	if (sUri == '' || sUri.charAt(0) == '#')
		return sBaseUri;

	var aUri = JsUnit.Uri.getUriComponents(sUri);
	if (aUri[0])	// scheme
		return sUri;

	var aBaseUri = JsUnit.Uri.getUriComponents(sBaseUri);
